/********************************************************************
	created:	2014/11/18
	created:	18:11:2014   17:00
	file base:	BasicFun
	file ext:	js
	author:		ixshells
	
	purpose:	封装一些关于EGS要用到的基本函数
*********************************************************************/


EGS.InitShader = EGS.Class(
{
    webgl : null,
    ...
    requestUrlText : function(url)
    {
        var xmlHttp = new XMLHttpRequest();
        xmlHttp.open("GET", "../../Shaders/" + url + "?t=" + Math.random(), false);
        xmlHttp.send();
        return xmlHttp.responseText;
    },

    loadFragmentShaderFromFile : function(scriptId)
    {
        var shaderScript = document.createElement("script");
        shaderScript.type = "x-shader/x-fragment";
        shaderScript.id = scriptId;
        shaderScript.innerHTML = this.requestUrlText(scriptId + ".fsh");
        var head = document.getElementsByTagName("body").item(0);
        head.appendChild(shaderScript);
    },

    //解析Shader代码
    shaderSourceFromScript : function(scriptID)
    {
    ...
    //编译Shader程序
    compileShader : function(webgl, shaderVectexCode, shaderFramentCode, vertexShaderObj, fragmentShaderObj)
    {
    ...
    //链接Shader程序
    linkShader : function(webgl, programObj, vertexShaderObj, fragmentShaderObj)
    {
    ...
    initShaders : function(webgl, vertexShaderId, framentShaderId, bindVariableData)
    {
        //生成shaderobject
        ...
        var vertextShaderCode = this.shaderSourceFromScript(vertexShaderId);
        var fragmentShaderCode = this.shaderSourceFromScript(framentShaderId);
        ...
        //编译
        this.compileShader(webgl, vertextShaderCode, fragmentShaderCode, vertexShaderObject, fragmentShaderObject);
        ...
        programObject = this.linkShader(webgl, programObject, vertexShaderObject, fragmentShaderObject);
